...
public sealed class Branch : MonoBehaviour
{
    ...
    private void SpawnLeavesReachedByVisibleStem()
    {
        ...
        while (nextLeafIndex < leafNodeCount)
        {
            ...
            CreateLeaf(nextLeafIndex, targetHeight, progress);
            nextLeafIndex++;
            nextLeafNodeSupportedSince = -1f;
        }
    ...
    private void CreateLeaf(int leafIndex, float height, float verticalProgress)
    {
        ...
        float azimuth = leafIndex * GoldenAngle
            + currGen * 53f
            + Random.Range(-7f, 7f);
