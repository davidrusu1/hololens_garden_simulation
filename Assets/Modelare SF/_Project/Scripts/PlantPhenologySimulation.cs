...
namespace ICI.PlantGrowth.Phenology
{
    ...
    public sealed class PlantPhenologySimulation : MonoBehaviour
    {
        ...
        /// <summary>
        /// Continues a paused simulation without resetting its numerical or
        /// visual DVS state. StartSimulation remains the explicit fresh start.
        /// </summary>
        public void ResumeSimulation()
        {
            ...
            if (profile == null)
            {
                statusMessage = "Lipsește profilul fenologic al plantei.";
                Debug.LogError(
                    $"[{nameof(PlantPhenologySimulation)}] No phenology profile is assigned.",
                    this);
                return;
            }
            ...
            mixedCropFieldController?.SetSimulationSpeed(
                GetEffectiveVisualGrowthSpeed());

            isRunning = true;
            mixedCropFieldController?.SetSimulationDay(
                currentDay + dayTimer / ReferenceSecondsPerSimulatedDay);
            ApplyVisualDevelopmentStage(
                GetInterpolatedVisualDevelopmentStage(
                    ReferenceSecondsPerSimulatedDay));
            statusMessage = "Simulare reluată fără resetarea DVS.";
        }
